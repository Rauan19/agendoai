...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loginMutation = useMutation({
    mutationFn: async (credentials: LoginData) => {
      ...
      try {
        // Chamada direta ao fetch para debug
        ...
        console.log("Headers login:", Object.fromEntries([...response.headers.entries()]));
        ...
        if (!response.ok) {
          ...
          try {
            ...
            // Se não puder analisar JSON, use o texto
            try {
              errorMessage = await response.text();
            } catch (e2) {
              // Mantenha o errorMessage original se não puder obter o texto
            }
          }
          throw new Error(errorMessage);
        }
        
        // Se foi bem-sucedido, retorne os dados
