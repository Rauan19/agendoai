...
export default function ProviderSchedulePage() {
  ...
  const { data: availabilityData, isLoading: isLoadingAvailability } = useQuery<AvailabilitySlot[]>({
    queryKey: ['/api/availability/provider', parsedProviderId],
    queryFn: () => {
      console.log(`Buscando disponibilidade do prestador: ${parsedProviderId}`);
      return fetch(`/api/availability/provider/${parsedProviderId}`).then(res => {
        ...
        return res.json().then(data => {
          ...
          return Array.isArray(data) ? data : data.availability || [];
        });
      }).catch(error => {
