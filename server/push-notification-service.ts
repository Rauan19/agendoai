...
class PushNotificationService {
  ...
  initialize(customPublicKey?: string, customPrivateKey?: string, customSubject?: string): void {
    ...
    // Verificar se as chaves são placeholders padrão (inválidas)
    const invalidKeys = [
    ...
    if (vapidPublicKey && vapidPrivateKey && 
        !invalidKeys.some(invalid => vapidPublicKey?.toLowerCase().includes(invalid) || vapidPrivateKey?.toLowerCase().includes(invalid))) {
      try {
        ...
        // Validar tamanho das chaves antes de configurar
        if (vapidPublicKey.length < 80 || vapidPrivateKey.length < 40) {
          throw new Error('Chaves VAPID muito curtas - provavelmente inválidas');
        }
        ...
        logger.info('Chaves VAPID inválidas detectadas. Continuando sem notificações push.');
        logger.info('Execute "npx tsx server/tools/generate-vapid-keys.ts" para gerar novas chaves válidas');
        this.initialized = false;
      }
    } else {
      logger.warn('Chaves VAPID não encontradas ou são placeholders inválidos');
      logger.info('Continuando sem serviço de notificações push');
      logger.info('Execute "npx tsx server/tools/generate-vapid-keys.ts" para gerar chaves VAPID válidas');
      this.initialized = false;
    }
  }
