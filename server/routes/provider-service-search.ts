...
router.get("/", async (req, res) => {
	try {
		...
		try {
			...
			// Buscar informações dos serviços solicitados usando tabela services
			const servicesInfo = await Promise.all(
				serviceIds.map(async (serviceId) => {
					try {
						...
						console.log(`Erro ao buscar serviço ${serviceId}:`, error)
						return null
					}
				})
			)
			...
			const validServices = servicesInfo.filter(s => s !== null)

			// Calcular duração total baseada nos serviços encontrados
			const totalDuration = validServices.reduce((total, service) => {
			...
			// Verificar se a duração total excede um limite razoável (ex: 10 horas por dia)
			const MAX_DURATION_PER_DAY = 600 // 10 horas em minutos
			console.log(
			...
			if (totalDuration > MAX_DURATION_PER_DAY && date) {
				...
				return res.status(400).json({
					...
					serviceDetails: validServices.map((s) => ({
						id: s.id,
						name: s.name,
						duration: s.duration,
...
async function filterProvidersByServices(
	...
	for (const provider of providers) {
		try {
			// Buscar todos os serviços do prestador usando a função correta
			const allServices = await storage.getProviderServicesByProviderId(provider.id)

			// Se não retornar array, converter para array
			...
			const offeredServiceIds = new Set(
				servicesArray.map((s) => s.serviceId || s.id)
			)
		...
		providers.map(async (provider) => {
			try {
				...
				if (
					...
					try {
						// Buscar disponibilidade por dia da semana usando função correta
						const availabilityResult = await storage.getAvailabilityByDay(provider.id, dayOfWeek)
						dayAvailability = Array.isArray(availabilityResult) ? availabilityResult : (availabilityResult ? [availabilityResult] : [])
						
						console.log(`Prestador ${provider.id}: Encontrada disponibilidade regular para dia ${dayOfWeek}:`, dayAvailability.length, 'períodos')
					} catch (availabilityError) {
						console.error(`Erro ao buscar disponibilidade do prestador ${provider.id}:`, availabilityError)
						dayAvailability = []
					}
				}
