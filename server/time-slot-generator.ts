...
export class TimeSlotGenerator {
  ...
  private calculateAvailableSlots(
    ...
    if (!availability || availability.length === 0) {
      console.log('Nenhuma disponibilidade encontrada para este prestador');
      return slots;
    }

    // Para cada período de disponibilidade
    for (const avail of availability) {
      if (!avail.isAvailable) continue;

      // Criar lista de períodos ocupados para este dia
      const occupiedPeriods: { start: number, end: number }[] = [];
      ...
      if (appointments && appointments.length > 0) {
        for (const appointment of appointments) {
          ...
          occupiedPeriods.push({ start: startMinutes, end: endMinutes });
        }
      }
      ...
      // Gerar slots usando a função existente
      const workingHours = {
